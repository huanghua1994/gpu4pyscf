...
def _kernel(mf, conv_tol=1e-10, conv_tol_grad=None,
    ...
    if hasattr(mf, 'mxp_df_level'):
        if mf.mxp_df_level < 0 or mf.mxp_df_level > 3:
    ...
    for cycle in range(mf.max_cycle):
        ...
        # TODO: need to tune
        if mf.mxp_df_level == 3 and rel_delta_e > 2e-6:
            mf.mxp_df_level = 3
        elif mf.mxp_df_level >= 2 and rel_delta_e > 5e-9:
            mf.mxp_df_level = 2
        elif mf.mxp_df_level >= 1 and rel_delta_e > 2e-11:
            mf.mxp_df_level = 1
        else:
            mf.mxp_df_level = 0
        if not use_cuda_jk_build_:
        ...
        log.info('cycle= %d E= %.15g  delta_E= %4.3g  norm_gorb= %4.3g  |ddm|= %4.3g',
    ...
    log.info(f"MxP DF level 0, 1, 2, 3 iters: {mxp_df_level_cnt}")
