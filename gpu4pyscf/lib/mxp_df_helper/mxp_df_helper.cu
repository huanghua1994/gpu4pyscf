...
)
{
    ...
    if (fp64_emu_bits > 0) compute_type = CUBLAS_COMPUTE_64F_EMULATED_FIXEDPOINT;
...
void DF_K_build_subtask2(
    ...
    void *padded_K_k, const int fp64_emu_bits = 0
)
{
    cublasComputeType_t compute_type;
    cudaDataType_t A_dtype, B_dtype, C_dtype, scale_dtype;
    ...
    {
        cublasGemmAlgo_t algo = CUBLAS_GEMM_DEFAULT;
        compute_type = CUBLAS_COMPUTE_64F_EMULATED_FIXEDPOINT;
...
void DF_K_build_fp64_work(
    ...
    const int K_mat_acc, const int fp64_emu_bits
)
{
    cublasEmulationStrategy_t orig_emu_strategy;
    cudaEmulationSpecialValuesSupport orig_sv_support;
    cudaEmulationMantissaControl orig_mantissa_control;
    int orig_mantissa_bits = 0;

    if (fp64_emu_bits > 0)
    {
        cublasGetEmulationStrategy(cublas_handle, &orig_emu_strategy);
        cublasGetEmulationSpecialValuesSupport(cublas_handle, &orig_sv_support);
        cublasGetFixedPointEmulationMantissaControl(cublas_handle, &orig_mantissa_control);
        cublasGetFixedPointEmulationMaxMantissaBitCount(cublas_handle, &orig_mantissa_bits);

        cublasSetEmulationStrategy(cublas_handle, CUBLAS_EMULATION_STRATEGY_PERFORMANT);
        cublasSetEmulationSpecialValuesSupport(cublas_handle, CUDA_EMULATION_SPECIAL_VALUES_SUPPORT_DEFAULT);
        cublasSetFixedPointEmulationMantissaControl(cublas_handle, CUDA_EMULATION_MANTISSA_CONTROL_FIXED);
        cublasSetFixedPointEmulationMaxMantissaBitCount(cublas_handle, fp64_emu_bits);
    }
    ...
    if (fp64_emu_bits > 0)
    {
        cublasSetEmulationStrategy(cublas_handle, orig_emu_strategy);
        cublasSetEmulationSpecialValuesSupport(cublas_handle, orig_sv_support);
        cublasSetFixedPointEmulationMantissaControl(cublas_handle, orig_mantissa_control);
        cublasSetFixedPointEmulationMaxMantissaBitCount(cublas_handle, orig_mantissa_bits);
    }
...
)
{
    if (cublas_handle == nullptr)
    {
        ...
        CUBLAS_CHECK( cublasSetMathMode(cublas_handle, CUBLAS_MATH_DISALLOW_REDUCED_PRECISION_REDUCTION) );
    }
