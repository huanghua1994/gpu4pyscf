...
#define GEMM_MAX_N     131072    // Max N size for a single GEMM call, need to tune
#define GEMM_MAX_K     131072    // Max K size for a single GEMM call, need to tune
...
void DF_K_build_subtask1(
    ...
    const T *cderi_i, const T *C_or_D_j, void *rho_K_k
)
{
...
void DF_K_build_subtask1_transpose(
    cudaStream_t stream, cublasHandle_t cublas_handle, const int use_Dmat,
    const int init_output, const int naux_blk, const int padded_nao, const int padded_nocc,
    const T *cderi_i, const T *C_or_D_j, void *rho_K_k_0, void *rho_K_k, 
    const int moduli, void *gemmul8_workspace, const int fp64_emu_bits = 0
)
{
    cublasComputeType_t compute_type;
    cudaDataType_t A_dtype, B_dtype, C_dtype, scale_dtype;
    ...
    if (fp64_emu_bits > 0)
    {
        ...
        for (int n_pos = 0; n_pos < n; n_pos += GEMM_MAX_N)
        {
            int curr_n = std::min(GEMM_MAX_N, n - n_pos);
            const double *curr_B = B_ + n_pos * ldB;
            double *curr_C = C_ + n_pos * ldC;
            time_breakdown = gemmul8::gemm<double>(
                cublas_handle, transA, transB, m, curr_n, k,
                alpha_, A_, ldA, curr_B, ldB, beta_, curr_C, ldC,
...
void DF_K_build_subtask2(
    cudaStream_t stream, cublasHandle_t cublas_handle, const int use_Dmat,
    const int init_output, const int naux_blk, const int padded_nao, const int padded_nocc,
    const T *cderi_or_rho_K_i, const T *rho_K_j, void *padded_K_k,
    const int moduli = 0, void *gemmul8_workspace = nullptr, const int fp64_emu_bits = 0
)
{
    cublasComputeType_t compute_type;
    cudaDataType_t A_dtype, B_dtype, C_dtype, scale_dtype;
    ...
    if (fp64_emu_bits > 0)
    {
        ...
        const double *B_ = static_cast<const double *>(B);
        double *C_ = static_cast<double *>(C);
        const int k_blk_size = std::min(k, GEMM_MAX_K);
...
void DF_K_build_fp64_work(
    ...
    void *gemmul8_workspace, const int fp64_emu_bits = 0
)
{
    ...
    if (fp64_emu_bits > 0)
    {
        DF_K_build_subtask1_transpose<double>(
            ...
            padded_cderi, padded_C_or_D_mat, rho_K_0_buf, rho_K_buf,
            moduli, gemmul8_workspace, fp64_emu_bits
        );
    } else {
...
void DF_JK_build_work(
    ...
    const int build_K, double *J_mat_sparse, double *K_mat,
    const size_t gemmul8_workspace_bytes, void *workbuf
)
{
    ...
    for (int i = 0; i < naux; i += naux_blk)
    {
        ...
        if constexpr(std::is_same_v<DType, double>)
        {
            DF_K_build_fp64_work(
                ...
                rho_K_buf, padded_K_buf, K_mat, K_mat_acc,
                gemmul8_workspace, fp64_emu_bits
            );
        }
        if constexpr(std::is_same_v<DType, float>)
        {
...
)
{
    ...
    do { \
        DF_JK_build_work<DType>( \
            ...
            build_K, J_mat_sparse_d, K_mat_d, \
