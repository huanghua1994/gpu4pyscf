...
non_flagship_gpu_mxp_strategies = {
    ...
    1: ('FP64', 1, 39),  # cuBLAS emulated FP64, 39 bits accuracy (~2e-12)
    2: ('FP64', 1, 31),  # cuBLAS emulated FP64, 31 bits accuracy (~4e-10)
    3: ('FP64', 1, 23),  # cuBLAS emulated FP64, 23 bits accuracy (~2e-7)
}
...
def mxp_df_level_to_split_dtype(mxp_df_level: int) -> Tuple[str, int, int]:
    ...
    """
    if mxp_df_level < 0 or mxp_df_level > 3:
        raise ValueError(f"Unsupported MxP DF level: {mxp_df_level}. Supported levels are 0, 1, 2, 3.")
    if is_flagship_gpu():
