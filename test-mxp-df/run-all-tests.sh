    ...
    for basis in "${basis_set[@]}"; do
        for calc_type in "${calc_types[@]}"; do
            for mxp_df_level in "${mxp_df_levels[@]}"; do
                ...
                echo "Running: python test-mxp-df.py $xyz_file $basis $calc_type --mxp_df_level $mxp_df_level --max_cycle $max_cycle, log to $log_file"
                python3 test-mxp-df.py "$xyz_file" "$basis" "$calc_type" "--mxp_df_level" "$mxp_df_level" "--max_cycle" "$max_cycle" &> "$log_file"
            done
        done
    done
done
