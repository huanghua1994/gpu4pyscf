...
export LD_PRELOAD=$HOME/scratch.huah_gpu/ozIMMU/build/libozimmu.so

# Warn-up running
echo "Warm-up running..."
for mxp_df_level in "${mxp_df_levels[@]}"; do
    for basis in "${basis_set[@]}"; do
        python3 test-mxp-df.py data/alkane/alkane_32.xyz "$basis" "--mxp_df_level" "$mxp_df_level" &> /tmp/mxp-df-warmup.log
    done
done
    ...
    for basis in "${basis_set[@]}"; do
        for calc_type in "${calc_types[@]}"; do
            for mxp_df_level in "${mxp_df_levels[@]}"; do
                ...
                echo Running: python3 test-mxp-df.py "$xyz_file" "$basis" "--calc_type" "$calc_type" \
                    "--xc" $xc "--mxp_df_level" "$mxp_df_level" "--max_cycle" "$max_cycle" \
                    "--run_verbose" $run_verbose "--conv_tol" $conv_tol, log to "$log_file"

                python3 test-mxp-df.py "$xyz_file" "$basis" "--calc_type" "$calc_type" \
                    "--xc" $xc "--mxp_df_level" "$mxp_df_level" "--max_cycle" "$max_cycle" \
